/* GAME PROGRAMMING A2 - 3D world with perlin noise, gravity and clouds, projectiles, timing, world boundaries
...
void collisionResponse() {
    ...
    if (world[(int)-px][(int)-py][(int)-pz] != 0){
        ...
        if (-px > 100 || -pz > 100 || -py > 50 || -px < 0 || -pz < 0 || -py < 0){
            setViewPosition(oldpx,oldpy,oldpz);
            return;
        }
        if (gravity == 1){
            if (world[(int)-px][(int)-(py+0.8)][(int)-pz] == 0){
                setViewPosition(px,py+0.18,pz);
                oldpy= py+0.18;
                oldpx = px;
                oldpz = pz;
                return;
...
void fire(int mob, float velocity, float angle, float x,float y,float z, float roty,time_t start){
    createMob(mob,x,y,z,roty);
    bullets[mob].active = 1;
    bullets[mob].mobNum = mob;
    bullets[mob].X = x;
    bullets[mob].Y = y;
    bullets[mob].Z = z;
    bullets[mob].roty = roty;
    bullets[mob].tStart = start;
    bullets[mob].vX = velocity*cos((3.14/180)*angle)*20;
    bullets[mob].vY = velocity*sin((3.14/180)*angle)*20;
...
void moveProjectiles(){
    ...
    for (i = 0; i < mobCount; i++){
        ...
        if (bullets[i].X > 100 || bullets[i].Z > 100 || bullets[i].X < 0 || bullets[i].Z < 0){
            hideMob(i);
            bullets[i].active = 0;
        }
        double dur = (double)(clock() - bullets[i].tStart) / CLOCKS_PER_SEC;
        float xPos = bullets[i].X - bullets[i].vX*sin((3.14/180)*bullets[i].roty)*dur;
        float zPos = bullets[i].Z - bullets[i].vX*cos((3.14/180)*bullets[i].roty)*dur;
        float yPos = bullets[i].Y + (bullets[i].vY*dur) - (3*dur*dur);
       
        if (world[(int)xPos][(int)yPos][(int)zPos] != 0){
            hideMob(i);
            collide((int)xPos,(int)yPos,(int)zPos);
            bullets[i].active = 0;
        }
        else{
            setMobPosition(i,xPos,yPos,zPos,bullets[i].roty);
        }
    }
}

void collide (int x, int y, int z){
    if ( x > 96 || z > 96 || x < 4 || z < 4){
        return;
    }
    int i,j;
    for (i = 0; i < 5; i++){
        for (j = 0; j < 5; j++){
            world[x+i-2][y][z+j-2] = 0;
        }
    }
    for (i = 0; i < 3; i++){
        for (j = 0; j < 3; j++){
            world[x+i-1][y-1][z+j-1] = 0;
        }
    }
    world[x][y-2][z] = 0;
}
...
void mouse(int button, int state, int x, int y) {
    ...
    if (state == GLUT_UP && button == GLUT_RIGHT_BUTTON){
        velocity += (float)(x-RMDX)*0.001;
        angle += (float)-(y-RMDY)*0.15;
        
        //Clamp velocity and angle to 0.0-1.0 and 0.0-90.0 respecitivly
        velocity = velocity < 1.0 ? velocity : 1.0;
        velocity = velocity > 0 ?  velocity : 0;
        angle = angle < 90.0 ? angle :90.0;
    ...
    if (state == GLUT_DOWN && button == GLUT_LEFT_BUTTON){
        ...
        //mobCount = 1;
        time_t start = clock();
        fire(curMob,velocity,angle,-px,-py,-pz,roty,start);
        curMob = (curMob + 1) % 10;
        if (mobCount < 10){
            mobCount++;
        }
...
static time_t last_time;
   ...
   if (testWorld) {
    ...
    /* your code goes here */
        float time_spent = (double)(clock() - last_time) / CLOCKS_PER_SEC;
        if (time_spent < 0.032){
            return;
        }
